...
// Chaque joueur démarre avec l'alphabet complet, mais les voyelles sont
// disponibles en plusieurs exemplaires (sinon on est vite bloqué).
const LETTER_COUNTS = {
  A: 3, E: 4, I: 3, O: 3, U: 2, Y: 2,
  B: 1, C: 1, D: 1, F: 1, G: 1, H: 1, J: 1, K: 1, L: 1, M: 1,
  N: 1, P: 1, Q: 1, R: 1, S: 1, T: 1, V: 1, W: 1, X: 1, Z: 1,
}
...
export function buildAlphabetHand() {
  ...
  Object.entries(LETTER_COUNTS).forEach(([letter, count]) => {
    for (let i = 0; i < count; i++) {
      hand.push({ id: `l${letter}${Math.random().toString(36).slice(2, 6)}`, syllable: letter })
    }
  })
  return hand
}
