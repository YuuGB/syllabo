...
import {
  buildAlphabetHand, loadTrie, normalize, VOWELS,
} from './gameData'
...
function scoreClosedWords(table, trie, filterPlayerId) {
  ...
  Object.values(table).forEach((w) => {
    ...
    w.letters.forEach((l) => {
      ...
      scores[l.playerId] = (scores[l.playerId] || 0) + bonus
    })
  })
  return scores
}
...
export default function App() {
    ...
    const t = setTimeout(() => {
      setSolo((prev) => {
        ...
        if (move.type === 'new') {
          ...
          table = { ...table, [newId]: { letters: [{ syllable: move.card.syllable, playerId: 'ai' }], closed: false } }
        } else {
          const w = table[move.wordId]
          table = { ...table, [move.wordId]: { ...w, letters: [...w.letters, { syllable: move.card.syllable, playerId: 'ai' }] } }
        }
        const aiHand = prev.aiHand
          .map((c) => (c.id === move.card.id ? { ...c, usesLeft: (c.usesLeft ?? 1) - 1 } : c))
          .filter((c) => (c.usesLeft ?? 1) > 0)
  ...
  async function startGame() {
    ...
    const updates = {
      status: 'playing', turnOrder: shuffle(playerIds), turnIndex: 0, scoresSubmitted: false,
    }
  ...
  async function rematch() {
    const playerIds = Object.keys(room.players || {})
    const updates = {
      turnOrder: shuffle(playerIds), turnIndex: 0, table: {}, scoresSubmitted: false,
    }
    playerIds.forEach((pid) => { updates[`players/${pid}/hand`] = buildAlphabetHand() })
    await update(ref(db, `rooms/${roomCode}`), updates)
  }
  ...
  async function playCard(wordId) {
    ...
    const result = await runTransaction(wordRef, (cur) => {
      ...
      return { ...cur, letters: [...cur.letters, { syllable: card.syllable, playerId, playerName: me.name }] }
    })
  ...
  async function afterSuccessfulPlay(card) {
    await consumeCardUse(card.id)
    setSelectedCardId(null)
    await advanceTurn()
  }

  async function consumeCardUse(cardId) {
    await runTransaction(ref(db, `rooms/${roomCode}/players/${playerId}/hand`), (hand) => {
      if (!hand) return hand
      return hand
        .map((c) => (c.id === cardId ? { ...c, usesLeft: (c.usesLeft ?? 1) - 1 } : c))
        .filter((c) => (c.usesLeft ?? 1) > 0)
    })
  }
  ...
  function startSolo() {
    ...
    setSolo({
      ...
      turn: 'human', selectedCardId: null, selectedWordId: null, over: false, finalScore: 0,
    })
    setScreen('solo')
  }
  ...
  function soloPlayCard(wordId) {
    ...
    if (wordId === null) {
      ...
      const newId = 'w' + Math.random().toString(36).slice(2, 9)
      table = { ...table, [newId]: { letters: [{ syllable: card.syllable, playerId: 'human' }], closed: false } }
    } else {
      ...
      table = { ...table, [wordId]: { ...w, letters: [...w.letters, { syllable: card.syllable, playerId: 'human' }] } }
    }

    const humanHand = solo.humanHand
      .map((c) => (c.id === card.id ? { ...c, usesLeft: (c.usesLeft ?? 1) - 1 } : c))
      .filter((c) => (c.usesLeft ?? 1) > 0)
    const next = { ...solo, table, humanHand, selectedCardId: null, selectedWordId: null }
          ...
          <div className="modal-overlay" onClick={() => setShowRules(false)}>
            <div className="modal-box" onClick={(e) => e.stopPropagation()}>
              ...
              <ul>
                ...
                <li>Les voyelles (A, E, I, O, U, Y) peuvent être jouées jusqu'à 3 fois chacune avant de quitter ta main ; les consonnes ne servent qu'une seule fois. Chaque lettre posée rapporte 1 point, quel que soit son nombre d'utilisations restant.</li>
                <li>Un mot clos rapporte, à chaque joueur qui y a contribué, 1 point par lettre posée — doublé si le mot final est valide dans le dictionnaire.</li>
      ...
      <div className="game-screen">
        ...
        <div className="hand">
          {solo.humanHand.map((c) => (
            ...
            >
              {c.syllable}
              {VOWELS.includes(c.syllable) && <span className="card-mult">{c.usesLeft}×</span>}
            </button>
          ))}
